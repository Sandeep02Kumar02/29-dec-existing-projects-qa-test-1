...
| 20 | **Accept `//double` (double-leading-slash / protocol-relative) `308` redirect** as a residual deviation | WSGI middleware or a custom `WSGIRequestHandler` that rewrites the raw path before routing; a `before_request`/custom WSGI wrapper | A `//`-prefixed request target is parsed as a protocol-relative URL at the **WSGI environ-construction layer** (the first segment becomes the authority and `PATH_INFO` empties) **before** routing, so Werkzeug issues a `308`; `url_map.merge_slashes=False` at both rule and map level has no effect (verified). The only fixes are WSGI-level path rewriting or a custom request handler — both forbidden here (decision #6 rejected `before_request`/custom WSGI wrappers; "add nothing the source lacks"/minimal-changes) | `//`-prefixed targets redirect `308` instead of returning `200`; ordinary single-slash paths are unaffected |
...
The following table enumerates every behavior that cannot be made byte-identical purely at the Flask application layer, together with the chosen resolution and its risk. These are the residual deviations referenced in the summary above, and together they give **100% coverage of the known departures from strict Node parity**. Each is a documented, low-risk divergence. The `Server`-header, startup-banner, exotic-verb, and `//double`-redirect items are also recorded as decisions #9, #10, #11, and #20 in §1 (and the log-suppression choice that shapes the startup-banner row as decision #19), viewed there through the decision-making lens; they are consolidated here, alongside the `HEAD` deviation, so that all residual deviations are documented together in one authoritative place.
...
| 5 | Double-leading-slash / protocol-relative request target (e.g. `//double`) | Handler runs for any request target; `//double` returns `200` with the fixed body | The WSGI environ builder treats a `//`-prefixed target as a protocol-relative URL (the first segment becomes the authority and `PATH_INFO` empties) **before** routing, so Werkzeug returns a `308` redirect; `url_map.merge_slashes=False` at both rule and map level does not change this (verified) | Accept as a documented residual (decision #20); the only fixes — WSGI-level path rewriting or a custom `WSGIRequestHandler` — are out of scope (decision #6 rejected `before_request`/custom WSGI wrappers; "add nothing the source lacks") | `//`-prefixed targets redirect `308` instead of `200`; ordinary single-slash paths are unaffected |
